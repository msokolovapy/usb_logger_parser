...
class TemperatureData():
	...
	def __init__(self, usb_logger,temperature_data):
		self._original_data = temperature_data 
		self._usb_logger = usb_logger
		self._data_coll_freq = self.determine_ave_data_coll_frequency()
		self._data_matrix_size = {'data_width':len(self._original_data[0]),	'data_length':len(self._original_data)}
	...
	def read_from(cls,file_path):
		...
		usb_logger = USBLogger(logger_id, serial_no, file_basename)
		return cls(usb_logger,temperature_data)
	...
	@property
	def usb_logger(self):
		return self._usb_logger
	

	# class ReportingService():
	# 	def __init__(self, temperature_data, spike_data):
	# 		self._temperature_data = temperature_data
	# 		self._spike_data = spike_data

	# 	def generate_reports(self):
	# 		xlsx_graph = XLSXGraphReport(self._temperature_data)
	# 		xlsx_spike_summary = XLSXSpikeReport(self._spike_data)
...
class USBLogger():
	def __init__(self, logger_id, serial_no, file_basename):
		self._id = logger_id
...
if __name__ == '__main__':	
	...
	temperature_data = [TemperatureData.read_from(file) for file in file_list]
	for data in temperature_data:
		print(f'summary of temp data for {data.usb_logger.id}')
		print(data.data_coll_freq)
		print(data.data_matrix_size)
		print(data.usb_logger.id)
		print(data.usb_logger.serial_number)
		print(data.usb_logger.file_basename)
		print(data.usb_logger.storage_condition)
